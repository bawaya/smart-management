...
import { getFuelReportData } from '@/lib/utils/report-calculations';
...
export default async function FuelReportPage({ searchParams }: Props) {
  ...
  const [data, company] = await Promise.all([
    getFuelReportData(tenantId, year, month),
    getCompanyInfo(tenantId),
  ]);

  return <FuelReport data={data} company={company} />;
}
