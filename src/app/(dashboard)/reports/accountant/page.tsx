...
import { getAccountantReportData } from '@/lib/utils/report-calculations';
...
export default async function AccountantReportPage({ searchParams }: Props) {
  ...
  const [data, company] = await Promise.all([
    getAccountantReportData(tenantId, year, month),
    getCompanyInfo(tenantId),
  ]);

  return <AccountantReport data={data} company={company} />;
}
